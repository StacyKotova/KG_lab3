...
layout (binding = 5 , rgba32f) uniform  imageBuffer uCube;
layout (binding = 6, rgba32f) uniform  imageBuffer uCubeInd;

layout (binding = 1, rgba32f) uniform  imageBuffer uTetra;
layout (binding = 0, rgba32f) uniform  imageBuffer uTetraInd;
...
bool Raytrace ( SRay ray, float start, float final, inout SIntersection intersect )
{
    ...
    /////CUBE
    for(int j = 0; j < imageSize(uCubeInd); j++)
	{
        int vertex1 =  int(imageLoad(uCubeInd, j).x);
        int vertex2 =  int(imageLoad(uCubeInd, j).y);
        int vertex3 =  int(imageLoad(uCubeInd, j).z);
        STriangle triangle = STriangle(imageLoad(uCube, vertex1).xyz, imageLoad(uCube, vertex2).xyz, imageLoad(uCube, vertex3).xyz, 1);
    ...
    /////Tetra
    for(int j = 0; j < imageSize(uTetraInd); j++)
	{
        int vertex1 =  int(imageLoad(uTetraInd, j).x);
        int vertex2 =  int(imageLoad(uTetraInd, j).y);
        int vertex3 =  int(imageLoad(uTetraInd, j).z);
        STriangle triangle = STriangle(imageLoad(uTetra, vertex1).xyz, imageLoad(uTetra, vertex2).xyz, imageLoad(uTetra, vertex3).xyz, 1);
        if(RayTriangleIntersection(ray, triangle.v1, triangle.v2, triangle.v3, test) && test < intersect.Time )
		{
            intersect.Time = test;
            intersect.Point = ray.Origin + ray.Direction * test;
